...
const CategoryListingsScreen = () => {
  ...
  const fetchAuctions = useCallback(async (isRefreshing = false) => {
    ...
    try {
      const res = await fetch(`http://192.168.0.4:8000/api/auctions/?category=${id}&ordering=${ordering}&page=${currentPage}`);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {auctions.length === 0 && !loading ? (
        <View style={styles.center}><Text>Bu kategoride ilan bulunamadı.</Text></View>
      ) : (
        <FlatList
          data={auctions}
